...
class Tournoi extends CI_Controller {
  ...
  private function tour_display($tour) {
    $texte = array('1' => 'huitième de finale',
                   '2' => 'quart de finale',
		   '3' => 'demi-finale',
		   '4' => 'finale');
    return $texte[$tour];
  ...
  public function valider_inscription()  {
    ...
    $this->form_validation->set_rules('mail', 'Adresse mail',
				      'trim|required|valid_email|is_unique[Joueur.J_mail]');
    $this->form_validation->set_rules('tel', 'Téléphone',
				      'trim|required|integer');
    $this->form_validation->set_rules('bureau', 'Bureau',
  ...
  public function les_resultats() {
	...
	$s_e1 = $m->R_score2;
      ...
      $liste[] = array('tour'     => $m->R_tour,
                       'display'  => $this->tour_display($m->R_tour),
                       'equipe1'  => $e_e1,
   		       'nom1'     => $n_e1[0]->E_nom,
       		       'joueurs1' => $j_e1,
		       'score1'   => $s_e1,
