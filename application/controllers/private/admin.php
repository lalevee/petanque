...
class Admin extends CI_Controller {
  ...
  public function valider_inscription() {
    $this->form_validation->set_rules('user1', 'Joueur 1',
                      'required|xss_clean');
    $this->form_validation->set_rules('user2', 'Joueur 2',
                      'required|xss_clean');
    if ($this->form_validation->run() == FALSE) {
      ...
      if ($this->input->post('user3') != '0') {
          $nbjoueur = 3;
          if ($this->input->post('user4') != '0') $nbjoueur = 4;
      }
      $nom_team = '';
  ...
  public function valider_paiement() {
    $this->form_validation->set_rules('joueur', 'Joueur',
                      'required|xss_clean');
    if ($this->form_validation->run() == FALSE) {
  ...
  public function mail_apayer() {
      ...
      $this->email->from('Organisation', 'Votre dévoué membre actif ASMIN');
      $this->email->to($data['mel']);
  ...
  public function valider_score() {
    ...
    if ($this->form_validation->run() == FALSE) {
        ...
        $tmp = $this->Rencontre->get_rencontre($this->input->post('equ1'),
                                               $this->input->post('equ2'));
          if (count($tmp) <= 0) {
            ...
            if ($rencontre->R_score1 > $rencontre->R_score2) $equipe = $this->input->post('equ2');
            else $equipe = $this->input->post('equ1');
  ...
  public function testmail() {
    ...
    $this->email->message('Testing é the email class.');
